...
async def call_character_identify(bot, event, config, image_url):
    bot.logger.info(f"接收来自用户{event.user_id}的角色识别指令")
    try:
        if not image_url:
            await bot.send(event, "未能获取到有效的图片，请重新发送！", True)
            return

        res = await anime_trace(image_url)
        if not res or not res[0]:
            await bot.send(event, "识别失败或未找到匹配角色，请稍后再试~", True)
            return

        forward_meslist = [Node(content=[Text(str(res[0]))])]
        if res[1]:
            forward_meslist.append(Node(content=[Text(str(res[1]))]))
        ai_tag = "是 (AI生成)" if res[2] else "否 (非AI生成)"
        forward_meslist.append(Node(content=[Text(f"AI创作检测：{ai_tag}")]))

        bot.logger.info("角色识别成功，正在发送合并转发消息")
        await bot.send(event, forward_meslist)
        return {"result": str(res)}

    except Exception as e:
        bot.logger.error(f"角色识别出错：{e}")
        await bot.send(event, "出错啦，请稍后再试~", True)
        traceback.print_exc()
...
def main(bot, config):
    ...
    async def startYouridentify(event: GroupMessageEvent):
        ...
        if event.pure_text == "识别" or (
            event.get("at")
            and event.get("at")[0]["qq"] == str(bot.id)
            and event.get("text") is not None
            and "识别" in event.get("text")[0]
        ):
